...
# Variable global para almacenar el último mensaje HTTP recibido
last_http_message = ""
...
# Endpoint HTTP para recibir datos del Arduino
@app.route('/api/sensor_data', methods=['POST'])
def receive_sensor_data():
    global last_http_message
    try:
        data = request.get_json()
        if not data:
            return jsonify({'error': 'No data provided'}), 400

        # Espera formato: {"humidity": float, "temperature": float, "n": float, "p": float, "k": float}
        ...
        last_http_message = f"{humidity},{temperature},{n},{p},{k}"
...
# API para obtener el último mensaje HTTP
@app.route('/api/last_http_message')
def get_last_http_message():
    return jsonify({'message': last_http_message})
...
if __name__ == '__main__':
    ...
    # Ejecuta la aplicación Flask (sin hilo UDP, ahora usa HTTP POST)
